...
| `PublishedDate` | Renders `publishedAt` as a compact `YYMMDD` date inside `<time dateTime title>` (article list only; the reader shows the full date). | `value: number` |
...
- Auto-mark-read reuses `toggleRead` with `isRead: true`, skipping the network call if the item is already read. It fires immediately on reader open (Enter/`o`/click). On >= 768px the reader pane renders the selection, so a displayed unread item — reached by deliberate nav (`j`/`k`/`n`/`g`/`G`) or as the landing selection of a deliberate view entry (sidebar click, `J`/`K` cycle, initial load) — becomes a *pending* item that is marked read only when the user switches away from it (mark-on-leave): to another item, by opening one, or by entering another feed/view. The view-entry flush is what marks a single-item list (and the last item read before switching feeds), which `j`/`k` could never leave. Manual `m` on the pending item cancels the pending mark, so an item deliberately kept unread stays unread. Incidental selection (row focus, e.g. clicking a row's star button) never auto-marks, and neither does the selection reshuffle after a refetch or search change drops rows — otherwise auto-refresh would chain-mark one untouched unread item per cycle. A reshuffle does, however, make the newly displayed row the pending item, so the first switch away from it still marks it.
